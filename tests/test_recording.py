...
class TestStartRecording:
    ...
    def test_start_recording_sets_flags(self, mock_state):
        ...
        assert mock_state["is_recording"] == True
...
class TestStopRecording:
    ...
    def test_stop_recording_with_actions(self, mock_state):
        ...
        with patch('recording.time.perf_counter', return_value=1.0), \
             patch('recording.save_recording') as mock_save:
            
            stop_recording(mock_state, update_overlay)
        
        assert mock_state["is_recording"] == False
...
class TestSaveRecording:
    ...
    def test_save_recording_success(self, mock_state, temp_recordings_dir):
        ...
        with patch('recording.RECORDINGS_DIR', temp_recordings_dir), \
             patch('recording.get_recordings_folder', return_value=temp_recordings_dir):
...
class TestPlaybackRecording:
    ...
    def test_playback_with_actions(self, mock_state):
        ...
        mock_state["recorded_actions"] = [
            ...
            {"type": "key_release", "time": 0.1, "key": keyboard.KeyCode.from_char('a')},
        ]
        
        with patch('recording.time.perf_counter', side_effect=[0, 0, 0.001, 0.1, 0.101, 0.2]), \
             patch('recording.time.sleep'), \
             patch('recording.keyboard_controller') as mock_kb, \
             patch('recording.mouse'):
