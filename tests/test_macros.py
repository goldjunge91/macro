...
def mock_dependencies():
    ...
    with patch('macros.time') as mock_time, \
         patch('macros.keyboard') as mock_keyboard, \
         patch('macros.mouse') as mock_mouse, \
         patch('macros.keyboard_controller') as mock_kb_ctrl, \
         patch('macros.move_mouse_horizontal') as mock_move_mouse, \
         patch('macros.SendInput') as mock_send_input, \
         patch('macros.Input') as mock_input, \
         patch('macros.Input_I') as mock_input_i, \
         patch('macros.KeyBdInput') as mock_kbd_input, \
         patch('macros.MouseInput') as mock_mouse_input, \
         patch('macros.click_mouse_fast') as mock_click_fast, \
         patch('macros.threading') as mock_threading:
...
class TestRunThrowMacro:
    ...
    def test_throw_macro_basic_flow(self, mock_state):
        ...
        with patch('macros.time.sleep') as mock_sleep, \
             patch('macros.time.perf_counter') as mock_perf, \
             patch('network.send_clumsy_hotkey') as mock_clumsy:
    ...
    def test_throw_macro_e_spam_loop(self, mock_state):
        ...
        with patch('macros.time.sleep') as mock_sleep, \
             patch('macros.time.perf_counter') as mock_perf, \
             patch('network.send_clumsy_hotkey'):
    ...
    def test_throw_macro_clumsy_deactivation(self, mock_state):
        ...
        with patch('macros.time.sleep'), \
             patch('macros.time.perf_counter') as mock_perf, \
             patch('network.send_clumsy_hotkey') as mock_clumsy, \
             patch('input_control.mouse'), \
             patch('input_control.move_mouse_horizontal'), \
             patch('input_control.tap_tab'), \
             patch('input_control.keyboard_controller'):
...
class TestRunThrowMacroV2:
    ...
    def test_throw_v2_basic_flow(self, mock_state):
        ...
        with patch('macros.time.sleep'), \
             patch('macros.time.perf_counter', side_effect=[i * 0.01 for i in range(100)]), \
             patch('macros.SendInput') as mock_send, \
             patch('macros.move_mouse_horizontal'), \
             patch('macros.Input'), \
             patch('macros.Input_I'), \
             patch('macros.MouseInput'), \
             patch('macros.KeyBdInput'), \
             patch('macros.ctypes'):
...
class TestRunComplexMacro:
    ...
    def test_complex_macro_already_running(self, mock_state):
        ...
        assert mock_state["is_running_macro"] == True
